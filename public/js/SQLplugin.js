$(function () {
    ...
    $('#schemasList').on('click', 'li.template-list__item', (event) => {
        ...
        $.get('/oracle/getSchemes')
            .done((schemeConfig) => {
                ...
                $('#name').val(name);
                $('#login').val(currentScheme.user);
                $('#password').val(currentScheme.password);
                $('#connectString').val(currentScheme.connectString);
            })
            .fail(() => {
    ...
    //удаление схемы
    $('#schemasList').on('click', 'img.template-list__remove', (event) => {
    ...
    $('#templateList').on('click', 'img.template-list__remove', (event) => {
    ...
    $('#currentSchemeTemplatesList').on('click', 'li.template-list__item', (event) => {
...
function renderSavedTemplates() {
    ...
    $.get('/fileAction/getLists?type=sql_template').done(templates => {
        ...
        templates.forEach(template => {
            allTemplateList.append(`<li class="template-list__item">${template}<img class="template-list__remove" src="../img/remove.png"></li>`);
            if (re.test(template)) {
                shortList.append(`<li class="template-list__item">${template.split('@')[0]}</li>`);
            }
        })
    });
}
    ...
    $.get('/oracle/getSchemes')
        .done((schemeConfig) => {
            ...
            Object.keys(schemeConfig).forEach(schemeName => {
                ...
                schemeList.append(`<li class="template-list__item">${schemeName}<img class="template-list__remove" src="../img/remove.png"></li>`);
            });
            renderSavedTemplates()
        })
}
